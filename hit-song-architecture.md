# calyra-ai Architecture Document v1.0
...
│       └── 20260501000000_hit_song.sql     # 新增：calyra-ai 表结构
...
- 英语: `calyra-ai.ai/create`, `calyra-ai.ai/song/xxx`
- 西班牙语: `calyra-ai.ai/es/create`, `calyra-ai.ai/es/song/xxx`
- API 路由不走 i18n: `calyra-ai.ai/api/generate/lyrics`
...
calyra-ai 的前端页面必须同时服务增长、信任和创作体验。不同页面的优先级不同，但所有页面都必须保持精致、克制、清晰的产品质感。

**公开增长页面：** SEO 闭环第一，视觉第二但必须精致。页面必须优先保证 SSR 可索引内容、结构化数据、内链、首屏核心信息、分享预览和 CTA 转化。视觉设计不能牺牲 SEO、性能和转化路径。适用页面：`/`、`/song/[id]`、`/pricing`、公开内容页、未来 SEO 词簇页。

**产品功能页面：** 用户任务效率第一，视觉必须精致但克制。页面要清晰表达当前状态、下一步操作、成本/积分影响、生成进度和错误恢复方式。适用页面：`/create`、`/dashboard`、音频生成流程、报告页。

**认证和设置页面：** 信任感、简洁和少干扰第一。页面要减少视觉噪音，突出登录、注册、找回密码、账户设置等核心动作。

**视觉风格：** 整体参考 Google / Material 3 的设计审美：清晰层级、克制留白、强可读性、响应式、轻动效、细腻交互。可参考 `app.superdesign.dev` 的现代工具感和模块化体验，但不复制视觉。

**禁止项：** 不做模板感 landing page，不使用大面积渐变、装饰性 orb、过度卡片堆叠、低信息密度 hero、牺牲移动端首屏效率的视觉效果。
...
- CLAUDE.md 生成（项目规范文档）
- 品牌改造：Logo → calyra-ai，Landing page 改造
...
- SEO 基础
  - 动态 `generateMetadata`
  - title 模板：`{songTitle} - AI Generated {style} Song | calyra-ai`
...
_calyra-ai Architecture v1.0 — 2026年5月_
