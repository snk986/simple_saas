...
**目标：** 完成 calyra-ai 的商业化闭环和增长数据基础，让定价、订阅、积分包、播放排行和百分比排名可以支撑付费转化与公开传播。
...
**目标：** 把当前通用 SaaS 套餐替换为 calyra-ai 的真实商业套餐，并让 `/pricing` 成为可索引、可信任、可转化的增长页面。
...
3. 重做 `/pricing` 页面信息架构：
   ...
   - 首屏明确展示 calyra-ai、积分如何消费、套餐差异、退款承诺、主 CTA。
