# Hit-Song Dev 环境上线排障记录

日期：2026-05-08

本文记录 Hit-Song `dev` 环境上线验证过程中遇到的问题、原因和解决办法。以后本项目的上线排障记录统一使用中文，并按以下结构整理：
...
如果继续刷新旧浏览器 tab 或旧 `hit-song-xxxx.vercel.app` 地址，可能仍然访问的是旧环境变量快照。
...
```text
hit-song-l9ltxo13q-snk986s-projects.vercel.app
```
...
```text
https://hit-song-dev.vercel.app
```

注意：

- `dev.hit-song.com` 需要拥有 `hit-song.com` 域名并配置 DNS，否则会显示 `Invalid Configuration`。
...
```env
BASE_URL=https://hit-song-dev.vercel.app
CREEM_SUCCESS_URL=https://hit-song-dev.vercel.app/dashboard
```

Supabase Auth：

```text
Site URL = https://hit-song-dev.vercel.app
Redirect URLs = https://hit-song-dev.vercel.app/**
```

Creem test webhook：

```text
https://hit-song-dev.vercel.app/api/webhooks/creem
```
...
```text
https://hit-song-dev.vercel.app/api/generate/lyrics
```
...
```env
BASE_URL=https://hit-song-dev.vercel.app
KIE_CALLBACK_URL=https://hit-song-dev.vercel.app/api/webhooks/kie
```
